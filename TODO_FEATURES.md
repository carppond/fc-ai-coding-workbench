...
> 历史功能规划。当前工作流为终端 OMP / Pi + Git 面板 + 文件编辑；以下目标不等同于当前实现状态，以代码和 README 为准。
> 创建时间：2026-02-26
...
**现状：** 已支持通过所选 OMP / Pi 生成英文提交说明，不调用提交命令；跟随界面语言生成仍是规划目标。
...
**实现思路：**
- 后端通过所选 CLI 的打印模式生成说明，diff 经标准输入传递，不保存会话并禁用工具和扩展
